...
@implementation SXMp3RecordWriter


- (BOOL)createFileWithRecorder:(SXAudioRecorder*)recoder;
{
    // mp3压缩参数
    _lame = lame_init();
...
- (BOOL)writeIntoFileWithData:(NSData*)data withRecorder:(SXAudioRecorder*)recoder inAQ:(AudioQueueRef)						inAQ inStartTime:(const AudioTimeStamp *)inStartTime inNumPackets:(UInt32)inNumPackets inPacketDesc:(const AudioStreamPacketDescription*)inPacketDesc
{
    ...
    // 此处上传二进制pcm流到服务器
    if (self.uploadLoader) {
        [self.uploadLoader uploadData:data];
    }
...
- (BOOL)completeWriteWithRecorder:(SXAudioRecorder*)recoder withIsError:(BOOL)isError
{
    //关闭就关闭吧。管他关闭成功与否
    if(_file){
        fclose(_file);
